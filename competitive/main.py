...
import competitive as competitive
...
def main():
    ...
    while True:
        try:
            num_neurons = int(input(f"Enter number of neurons (default={num_neurons}): ") or num_neurons)
            if num_neurons <= 0:
                print("The learning rate must be a positive number!")
            else:
                break
            ...
            print("Invalid input! Please enter a number.")
    competitive.competitive(X, num_neurons=num_neurons, num_iterations=num_epochs, learning_rate=lr)
